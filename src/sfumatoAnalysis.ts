...
export class SfumatoAnalysis {
    private readonly ambiguousWords = [
        'data', 'info', 'temp', 'tmp', 'obj', 'item', 'thing', 'stuff',
        'value', 'val', 'var', 'x', 'y', 'z', 'i', 'j', 'k',
    ...
    private analyzeVariables(line: string, lineNumber: number, languageId: string): SfumatoIssue[] {
        ...
        const variablePatterns = {
            ...
            'java': [
                /(?:int|String|boolean|double|float|char|long)\s+([a-zA-Z_$][a-zA-Z0-9_$]*)/g
            ]
        };

        const patterns = variablePatterns[languageId as keyof typeof variablePatterns] || variablePatterns['javascript'];
    ...
    private analyzeFunctions(line: string, lineNumber: number, languageId: string): SfumatoIssue[] {
        ...
        const patterns = functionPatterns[languageId as keyof typeof functionPatterns] || functionPatterns['javascript'];
    ...
    private analyzeComments(line: string, lineNumber: number, languageId: string): SfumatoIssue[] {
        ...
        const patterns = commentPatterns[languageId as keyof typeof commentPatterns] || commentPatterns['javascript'];
    ...
    private followsNamingConvention(name: string, languageId: string): boolean {
        ...
        const pattern = conventions[languageId as keyof typeof conventions] || conventions['javascript'];
    ...
    private containsActionVerb(functionName: string): boolean {
        const actionVerbs = [
            'get', 'set', 'create', 'delete', 'update', 'add', 'remove',
            'calculate', 'process', 'validate', 'parse', 'format',
            'convert', 'transform', 'generate', 'build', 'execute',
            'handle', 'manage', 'check', 'verify', 'confirm'
        ];
