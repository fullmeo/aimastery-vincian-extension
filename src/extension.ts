// import { ReaperIntegration } from "./vscode-reaper-integration"; // Disabled - WIP feature
...
class EnhancedLogger {
  ...
  private config?: UnifiedConfigurationManager;

  private constructor() {
    this.outputChannel = vscode.window.createOutputChannel('AIMastery - Unified Extension');
  }

  // Initialize config manager after it's created (circular dependency workaround)
  setConfigManager(config: UnifiedConfigurationManager): void {
    this.config = config;
  }

  private get analyticsEnabled(): boolean {
    // Default to FALSE for privacy (opt-in, not opt-out)
    return this.config?.get('telemetry.enabled', false) || false;
  }
  ...
  private async trackEvent(event: string, data?: any): Promise<void> {
    if (!this.analyticsEnabled) {
      // Log that telemetry is disabled (only in output channel, not sent anywhere)
      this.outputChannel.appendLine(`[TELEMETRY DISABLED] Event '${event}' not tracked (user privacy respected)`);
      return;
    }

    try {
      // Envoi analytics asynchrone (fail silently)
      // TODO: Implement with user consent
      // Must include privacy notice and opt-in dialog
      this.outputChannel.appendLine(`[TELEMETRY] Event '${event}' tracked (anonymized)`);
    } catch (error) {
...
class UserProgressionManager {
  ...
  private async checkProgressionMilestones(
    ...
    if (!this.config.get('notifications.showMilestones', true)) {
      return;
    }

    // Get configurable notification delay
    const notificationDelay = this.config.get('notifications.delay', 2000);
    ...
    if (action === 'audio_analyzed' && progress.audioAnalyses === 1) {
      ...
      }, notificationDelay * 0.75); // Slightly faster for second milestone
    }
    ...
    if (progress.codeAnalyses >= 3 && progress.audioAnalyses >= 3) {
      ...
      }, notificationDelay * 0.5); // Faster for engaged users
    }
  }
      ...
      return Promise.resolve([
        {
          ...
          children: this.results.map(r => ({
            label: `Analysis ${new Date(r.timestamp).toLocaleTimeString()}`,
            tooltip: `Type: ${r.analysisType}`,
            description: `Score: ${r.codeHealth?.score || 'N/A'}`,
          })),
        },
      ]);
    }
  ...
  getChildren(element?: any): Thenable<any[]> {
    if (!element) {
      return Promise.resolve([
        {
          type: 'root',
          label: 'Vincian Analysis',
          children: this.results.map(r => ({
            label: r.fileName || 'Unknown',
            tooltip: `Vincian Score: ${r.vincianScore?.toFixed(1) || 'N/A'}`,
            description: `${r.vincianScore?.toFixed(0) || 0}/100`,
          })),
        },
      ]);
    }
          ...
          children: [
            {
              ...
              command: {
                command: AIMasteryConstants.COMMANDS.AUTO_FIX,
                title: 'Auto-fix',
              },
            },
            {
              label: '⚡ Self-improve',
              command: {
                command: AIMasteryConstants.COMMANDS.SELF_IMPROVE,
                title: 'Self-improve',
              },
            },
          ],
...
class AIMasteryExtensionManager {
  ...
  async activate(): Promise<void> {
    ...
    try {
      ...
      // 2. Initialiser les providers TreeView
      await this.initializeProviders();

      // 3. Enregistrer toutes les commandes
      await this.registerAllCommands();

      // 4. Configurer les tâches d'arrière-plan
      await this.setupBackgroundTasks();

      // 5. Vérifier le statut utilisateur
    ...
    await vscode.window.withProgress(
      ...
      async (progress, token) => {
        ...
        await this.progressManager.trackUserAction('workspace_analyzed', {
          healthScore: analysis.healthScore,
          linesAnalyzed: analysis.analysisMetadata?.linesAnalyzed || 0,
        });
      }
    );
  }
