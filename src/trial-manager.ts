...
export class TrialManager {
    ...
    static showAdvancedAnalytics(): void {
        ...
        <head>
            <style>
                body { font-family: 'Segoe UI'; padding: 20px; background: #1e1e1e; color: #fff; }
                .header { text-align: center; margin-bottom: 30px; background: linear-gradient(135deg, #FFD700 0%, #FFA500 100%); padding: 20px; border-radius: 10px; color: #000; }
                .chart-container { background: #2d2d2d; padding: 20px; border-radius: 10px; margin: 15px 0; }
                .metric-row { display: flex; justify-content: space-between; margin: 10px 0; padding: 10px; background: #3d3d3d; border-radius: 5px; }
                .creator-badge { background: #FFD700; color: #000; padding: 5px 15px; border-radius: 15px; font-weight: bold; }
        ...
        <body>
            ...
            <div class="chart-container">
                ...
                <div class="metric-row"><span>Total Revenue:</span><span class="value">€${totalRevenue}</span></div>
                <div class="metric-row"><span>Monthly Growth:</span><span class="value">+23%</span></div>
                <div class="metric-row"><span>Conversion Rate:</span><span class="value">${conversionRate}%</span></div>
            </div>
            <div class="chart-container">
                <h3>🎺 Personal Creator Usage</h3>
                <div class="metric-row"><span>Creator Analyses:</span><span class="value">${personalUsage}</span></div>
                <div class="metric-row"><span>Average Score:</span><span class="value">8.4/10</span></div>
            </div>
        </body>
        </html>`;
    }
...
🎯 Analyses perso : ${personalUsage}

Building in public avec ma propre extension ! 🚀
...
📊 Score Vincien : ${analysisResult.score || '8.7'}/10
🎨 Principes détectés : ${analysisResult.principles?.join(', ') || 'Mouvement, Équilibre, Harmonie'}
💰 Revenus extension : €${totalRevenue}
📈 Taux conversion : ${conversionRate}%
🎯 Mes analyses : ${personalUsage}
