...
const AIMASTERY_PLANS = {
    ...
    social_pack: {
        ...
        price: 9,
        price_id: "price_1RSaBkH4ErDlKYxlGgcmW7sS",
        monthly_credits: 100,
        trial_days: 7
    },
    pro_vincien: {
        name: "Pro Vincien",
        price: 15,
        price_id: "price_NOUVEAU_PRICE_ID_15EUR",
...
async function createPaymentIntent(productType, analysisData) {
    ...
    const paymentIntent = await stripe.paymentIntents.create({
        ...
        metadata: {
            ...
            priceEuros: pricing[productType] / 100,
...
async function handleUpgradeFlow(productType, analysisId) {
    ...
    const panel = vscode.window.createWebviewPanel('aimasteryFreemium', 'AIMastery - Choisissez votre plan', vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
...
async function createStripeCheckout(planType, userId) {
    try {
        ...
        */
        vscode.window.showInformationMessage(`💳 ${plan.name} checkout ready! ${plan.trial_days} days free trial. Backend deployment needed.`, 'View Analytics').then(selection => {
...
function getFreemiumWebviewContent(currentPlan, availableUpgrades) {
    return `
    <html>
    <head>
        <style>
...
//# sourceMappingURL=stripe_integration.js.map